...
class ContextSavingsBenchmarkTest(IsolatedRegistryTest):
    """The README "Proof" table is generated by bench_context_savings.py.

    Regression: the benchmark indexed result["savings"] directly, but bundle()
    omits that block when a task routes nothing. Any index without a match for
    all six fixed tasks crashed the documented reproduce command with a
    KeyError, so the published proof could not be regenerated.
    """
    ...
    def test_benchmark_skips_tasks_that_route_nothing(self) -> None:
        ...
        # The table renderer must tolerate the partial report too.
        with contextlib.redirect_stdout(io.StringIO()) as rendered:
            benchmark.print_table(report)
